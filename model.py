...
import torch
import spikefetch_pcpage_functions as sf_pcpage
from collections import OrderedDict

class MLPrefetchModel_spikefetch_pcpage(object):
    '''
    ...
    def generate(self, data):
        ...
        # create snn
        single_network = sf_pcpage.CreateNetwork(pattern_length, confidence_threshold, min_confidence, delta_range_length, neuron_numbers, timestamps, input_intensity)
        ...
        for (instr_id, cycle_count, load_addr, load_ip, llc_hit) in data:
            ...
            is_correct, label_removal_counter = sf_pcpage.check_hit(training_table, single_network.prediction_table, pc, page, offset, label_removal_counter)
            if is_correct:
            ...
            if pc in training_table:
                ...
                if page in inner_dict:
                    delta_pattern = sf_pcpage.calculate_deltas(inner_dict[page], offset)
                else:
            ...
            # if output_neurons:
            pc_removal_counter, page_removal_counter = sf_pcpage.update_training_table(pc, page, offset, output_neurons, delta_pattern, training_table, offset_prediction, pc_removal_counter, page_removal_counter)
...
Model = MLPrefetchModel_spikefetch_pcpage
