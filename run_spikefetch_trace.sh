...
find "./traces" -type f -name "*.txt.xz" | while read xz; do
  ...
  if [[ "$file_variable" == "$file_cc" ]]; then
    ./ml_prefetch_sim.py run ./traces/"$file_variable".trace.gz --prefetch ./spikefetch_prefetches_traces/prefetch_14th_rerun_"$file_variable".txt --num-instructions 20700644
  fi
  ...
  if [[ "$file_variable" == "$file_bfs10" ]]; then
    ./ml_prefetch_sim.py run ./traces/"$file_variable".trace.gz --prefetch ./spikefetch_prefetches_traces/prefetch_14th_rerun_"$file_variable".txt --num-instructions 61219343
  fi
